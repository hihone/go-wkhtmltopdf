...
Note: You can also ignore the internal buffer and let wkhtmltopdf write directly to disk if required for large files, or use the [SetOutput](https://godoc.org/github.com/hihone/go-wkhtmltopdf#PDFGenerator.SetOutput) method to pass any `io.Writer`.
...
```
go get -u github.com/hihone/go-wkhtmltopdf
```
...
As mentioned before, you can provide one document from stdin, this is done by using a [PageReader](https://godoc.org/github.com/hihone/go-wkhtmltopdf#PageReader "GoDoc") object as input to AddPage. This is best constructed with  [NewPageReader](https://godoc.org/github.com/hihone/go-wkhtmltopdf#NewPageReader "GoDoc") and will accept any io.Reader so this can be used with files from disk (os.File) or memory (bytes.Buffer) etc.  
...
For an example of running this in AWS Lambda see https://github.com/hihone/go-wkhtmltopdf-lambda
